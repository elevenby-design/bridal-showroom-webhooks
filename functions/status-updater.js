...
async function getCustomerByEmail(email) {
  const resp = await fetch(`https://${SHOPIFY_SHOP_DOMAIN}/admin/api/2023-10/customers/search.json?query=email:${encodeURIComponent(email)}` ,{
    headers: {
...
async function getCustomerMetafields(customerId) {
  const resp = await fetch(`https://${SHOPIFY_SHOP_DOMAIN}/admin/api/2023-10/customers/${customerId}/metafields.json`, {
    headers: {
      'X-Shopify-Access-Token': SHOPIFY_ACCESS_TOKEN,
      'Content-Type': 'application/json'
    }
