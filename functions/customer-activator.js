...
  try {
    // Build tags array - only include role + 'bridal-party' (no bride-only tags)
    const tags = ['bridal-party'];
    
...
            break;
          default:
            if (role && typeof role === 'string') tags.push(role);
        }
      });
    }
    
    // Get existing customer to preserve existing non-bride tags and remove any accidental bride tag
    const existingCustomer = await getCustomerById(customerId);
    const existingTags = existingCustomer ? existingCustomer.tags.split(',').map(tag => tag.trim()).filter(tag => tag) : [];
    
    // Remove any bride-only tags from existing set for invitees
    const filteredExisting = existingTags.filter(t => t !== 'showroom-bride');
    
    // Combine existing tags with new tags, avoiding duplicates
    const allTags = [...new Set([...filteredExisting, ...tags])];
    
